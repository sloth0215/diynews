# my-quiet-desk: AI 기반 멀티 플랫폼 RSS 피드 통합 관리 시스템
...
### 한 줄 설명
**Twitter(X), YouTube, 블로그에서 RSS 피드를 자동 수집하고, OpenAI를 활용하여 이벤트 일정을 추출해 캘린더로 표시하는 통합 피드 관리 웹 애플리케이션**
...
```python
# 검색 기능 추가
def search_posts(keyword: str, category: str = None):
    """
    게시물 검색
    - 전문 검색 (title, content, summary)
    - 카테고리 필터
    - 날짜 범위 필터
    """
    pass
...
# 스케줄 기능 추가
def schedule_daily_sync(time: str = "09:00"):
    """
    매일 정해진 시간에 자동 동기화
    - APScheduler 활용
    - 로그 저장
...
1. **프롬프트 엔지니어링**
   ...
   - 온도(Temperature) 조정의 영향

2. **API 통합**
   - 여러 외부 API의 응답 구조 다루기
   - API 한계 극복하기 (TwitterAPI.io 이미지 추출)
   - 비용 최적화 (max_entries 제한)
...
이 프로젝트는 **AI를 활용한 실제 서비스 개발**의 전체 과정을 경험했습니다:

✅ **기획** → 여러 플랫폼의 피드를 통합하는 아이디어
✅ **설계** → Firebase + Flask + React 스택 선택
✅ **개발** → Claude AI와의 협력 개발
✅ **배포** → 로컬 실행 가능한 완성도
✅ **개선** → 문제 해결 및 최적화

특히 **프롬프트 엔지니어링**을 통해 OpenAI를 효과적으로 활용하여 복잡한 NLP 작업(날짜 추출, 일정 감지)을 안정적으로 구현했습니다.

---

*이 README는 학습용 프로젝트로서 교육 목적으로 작성되었습니다.*